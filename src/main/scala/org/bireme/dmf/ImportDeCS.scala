...
object ImportDeCS {
  ...
  private def convertDoc(rec: Record): Try[Document] = {
    Try {
      ...
      doc.add(new StringField("descriptor_en", descrEn, Field.Store.YES))
      doc.add(new StringField("descriptor_es", descrEs, Field.Store.YES))
      doc.add(new StringField("descriptor_pt", descrPt, Field.Store.YES))
      doc.add(new StringField("descriptor_fr", descrFr, Field.Store.YES))
